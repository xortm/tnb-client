...
export default Ember.Controller.extend(dicttypeValidations, {
    ...
    refreshStaffList: function() {
        var route = App.lookup('route:business.mainpage.member-type');
    ...
    actions: {
        ...
        detailCancel: function() {
            ...
            if (id && editMode == 'edit') {
                ...
                mainpageController.switchMainPage('member-type', {});
            }
        },
        //存储
        savedicttype() {
                    ...
                    if (dicttypeModel.get('errors.length') === 0) {
                        ...
                        dicttypeModel.save().then(function(dicttype) {
                          ...
                          if(error.code==="4" || error.code==="12"){
                              App.lookup("controller:business.mainpage").showAlert("该会员等级已存在");
        ...
        delById: function() {
            ...
            var _self = this;
            App.lookup('controller:business.mainpage').showConfirm("是否确定删除此会员等级", function() {
                ...
                dicttype.save().then(function() {
                  ...
                  let error = err.errors[0];
                  if(error.code==="8"){
                      App.lookup("controller:business.mainpage").showAlert("该会员等级已有老人选择,不可删除");
