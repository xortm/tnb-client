...
export default Ember.Controller.extend({
  ...
  actions:{
    toDetailPage(role){
      if(role){
        let id=role.get('id');
        this.get("mainController").switchMainPage('role-set-detail',{id:id,editMode:"edit"});
      }else{
        this.get("mainController").switchMainPage('role-set-detail',{editMode:"add",id:''});
      }
    },
    toSetting(role){
      let id=role.get('id');
      this.get("mainController").switchMainPage('role-set',{id:id});
    },
    //保存备注
