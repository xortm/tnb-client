...
import ServiceitemValidations from '../../../validations/home-service';
...
export default Ember.Controller.extend(ServiceitemValidations,ServicemerchValidations,{
  ...
  actions:{
    uploadSucc: function(response) {
        ...
        this.get("serviceitemModel").set("picture", res.relativePath);
    },
    //选择物品
    selectMerch(merch){
    ...
    detailCancel:function(){
      ...
      var editMode=this.get('editMode');
        ...
        if(editMode=='add'){
          ...
          mainpageController.switchMainPage('home-project');
        }
    ...
    cancelPassSubmit(serviceitem){
      ...
      serviceitem.save().then(function() {
        ...
        if(error.code==='8' || error.code==='9'){
          App.lookup('controller:business.mainpage').showAlert("已预约的居家项目不能删除！");
        }
      });
		},
  }
