    ...
    actions:{
      ...
      toDetailPage(bill){
        ...
        if(bill.get('billStatus.typecode')==constants.billStatus1||bill.get('billStatus.typecode')==constants.billStatus2){
          this.get("mainController").switchMainPage('cost-detail',{id:id,editMode:"readOnly"});
        }else{
          this.get("mainController").switchMainPage('cost-detail',{id:id,editMode:"detail"});
        }
      },
      selectStatus(status){
        ...
        App.lookup("route:business.mainpage.cost-management").doQuery();
      },
      toEdit(bill){
        let id = bill.get('id');
        this.get("mainController").switchMainPage('cost-detail',{id:id,editMode:"edit"});
      },
      toAdd(){
